...
module.exports.run = async (bot, message, args) => {//Manyak*
    ...
    let result = Math.floor((Math.random() * replies.length));//Manyak*

    let gifembed = new Discord.MessageEmbed()//Manyak  
        .setTitle("Komik Paylaşımlar:")//Manyak*
        .setColor("PURPLE")//Manyak*
        .setFooter(`${message.author.username} `, message.author.avatarURL)//Manyak*
        .setImage(replies[result]);//Manyak*

    message.channel.send(gifembed);//Manyak*
};

exports.conf = {
    enabled: true,//Manyak*
    guildOnly: false,//Manyak*
    aliases: ['vine'],//Manyak*
    permLevel: 0//Manyak*
};
...
exports.help = {//Manyak*
    name: 'vine',//Manyak*
    description: 'Rastgele komik paylaşımlar atar.',//Manyak*
    usage: 'kp'//Manyak*
};//Many