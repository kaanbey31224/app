...
exports.run = async(client, message, args, ayar, emoji) => {
 if (!message.member.roles.cache.has("783639441839357962") && !message.member.hasPermission('ADMINISTRATOR')) return message.channel.send(new Discord.MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setColor(0x00bfff).addField(`\`Malesef yetkin yetmiyor!\``)).then(m => m.delete({timeout: 7000}));
  ...
  if (message.member.roles.highest.position <= uye.roles.highest.position) return message.channel.send(new Discord.MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setColor(0x00bfff).setDescription(`\`Bu kişi ya senle aynı yetkide yada senden üst yetkili \`).then(x => x.delete({timeout: 5000}));
  ...
  client.channels.cache.get("776784024558895114").send(new Discord.MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setColor(0x00bfff).setDescription(`${uye} Üyenin cezasını kaldırdı! \n\n• Kaldıran Yetkili: <@!${message.author.id}> \`${message.author.id}\`\n• Cezası kaldırılan Kullanıcı: <@!${uye.id}> \`${uye.id}\``)).catch();
};
exports.conf = {
    enabled: true,
    guildOnly: false,
